...
const types = {
  ...
  _do: createKeyword("افعل", {
    isLoop,
    beforeExpr
  }),
  _else: createKeyword("غير", {
    beforeExpr
  }),
  _finally: createKeyword("اخيرا"),
  _for: createKeyword("تكرار", {
    isLoop
  }),
  _function: createKeyword("مهمة", {
    startsExpr
  }),
  _if: createKeyword("اذا"),
  _return: createKeyword("ارجاع", {
    beforeExpr
  }),
  ...
  _while: createKeyword("طالما", {
    isLoop
  }),
  _with: createKeyword("with"),
  _new: createKeyword("جديد", {
    beforeExpr,
    startsExpr
  }),
  ...
  _super: createKeyword("ابي", {
    startsExpr
  }),
  _class: createKeyword("صنف", {
    startsExpr
  }),
  _extends: createKeyword("يرث", {
    beforeExpr
  }),
  ...
  _typeof: createKeyword("نوع", {
    beforeExpr,
    prefix,
    startsExpr
  }),
  _void: createKeyword("عقيم", {
    beforeExpr,
    prefix,
    startsExpr
  }),
...
function isIteratorStart(current, next) {
  return current === atSign && next === atSign;
}
...
var flow = (superClass => class extends superClass {
  ...
  getTokenFromCode(code) {
    ...
    if (code === leftCurlyBrace && next === verticalBar) {
      return this.finishOp(types.braceBarL, 2);
    } else if (this.state.inType && (code === greaterThan || code === lessThan)) {
  ...
  readToken_mult_modulo(code) {
    ...
    if (code === asterisk && next === slash && this.state.hasFlowComment) {
  ...
  readToken_pipe_amp(code) {
    ...
    if (code === verticalBar && next === rightCurlyBrace) {
  ...
  skipFlowComment() {
    ...
    while ([space, tab].includes(this.input.charCodeAt(pos + shiftToFirstNonWhiteSpace))) {
...
var jsx = (superClass => class extends superClass {
      ...
      switch (ch) {
        ...
        case leftCurlyBrace:
          if (this.state.pos === this.state.start) {
            if (ch === lessThan && this.state.exprAllowed) {
              ++this.state.pos;
  ...
  jsxReadNewLine(normalizeCRLF) {
    ...
    if (ch === carriageReturn && this.input.charCodeAt(this.state.pos) === lineFeed) {
      ++this.state.pos;
      out = normalizeCRLF ? "\n" : "\r\n";
    } else {
  ...
  jsxReadWord() {
    ...
    } while (isIdentifierChar(ch) || ch === dash);
  ...
  parseExprAtom(refShortHandDefaultPos) {
    ...
    } else if (this.isRelational("<") && this.input.charCodeAt(this.state.pos) !== exclamationMark) {
  ...
  getTokenFromCode(code) {
    ...
    if (context === types$1.j_oTag || context === types$1.j_cTag) {
      ...
      if ((code === quotationMark || code === apostrophe) && context === types$1.j_oTag) {
        return this.jsxReadString(code);
      }
    }

    if (code === lessThan && this.state.exprAllowed && this.input.charCodeAt(this.state.pos + 1) !== exclamationMark) {
      ++this.state.pos;
      return this.finishToken(types.jsxTagStart);
    }
...
var placeholders = (superClass => class extends superClass {
  ...
  getTokenFromCode(code) {
    if (code === percentSign && this.input.charCodeAt(this.state.pos + 1) === percentSign) {
...
const questionMark = 1567;
const breakKeyword = "خروج";
...
const forbiddenNumericSeparatorSiblings = {
  decBinOct: [dot, uppercaseB, uppercaseE, uppercaseO, underscore, lowercaseB, lowercaseE, lowercaseO],
  hex: [dot, uppercaseX, underscore, lowercaseX]
};
const allowedNumericSeparatorSiblings = {};
allowedNumericSeparatorSiblings.bin = [digit0, digit1];
allowedNumericSeparatorSiblings.oct = [...allowedNumericSeparatorSiblings.bin, digit2, digit3, digit4, digit5, digit6, digit7];
allowedNumericSeparatorSiblings.dec = [...allowedNumericSeparatorSiblings.oct, digit8, digit9];
allowedNumericSeparatorSiblings.hex = [...allowedNumericSeparatorSiblings.dec, uppercaseA, uppercaseB, uppercaseC, uppercaseD, uppercaseE, uppercaseF, lowercaseA, lowercaseB, lowercaseC, lowercaseD, lowercaseE, lowercaseF];
class Token {
  constructor(state) {
...
class Tokenizer extends LocationParser {
  ...
  skipLineComment(startSkip) {
    ...
    if (this.state.pos < this.length) {
      while (ch !== lineFeed && ch !== carriageReturn && ch !== lineSeparator && ch !== paragraphSeparator && ++this.state.pos < this.length) {
        ch = this.input.charCodeAt(this.state.pos);
      }
    }
  ...
  readToken_interpreter() {
    ...
    if (ch !== exclamationMark) return false;
  ...
  readToken_mult_modulo(code) {
    let type = code === asterisk ? types.star : types.modulo;
    let width = 1;
  ...
  readToken_plus_min(code) {
    ...
    if (next === code) {
      if (next === dash && !this.inModule && this.input.charCodeAt(this.state.pos + 2) === greaterThan && (this.state.lastTokEnd === 0 || lineBreak.test(this.input.slice(this.state.lastTokEnd, this.state.pos)))) {
  ...
  readToken_question() {
    ...
    if (next === questionMark && !this.state.inType) {
      if (next2 === equalsTo) {
        this.finishOp(types.assign, 3);
      } else {
  ...
  getTokenFromCode(code) {
    switch (code) {
      ...
      case colon:
        if (this.hasPlugin("functionBind") && this.input.charCodeAt(this.state.pos + 1) === colon) {
    ...
    this.raise(this.state.pos, `Unexpected character '${String.fromCodePoint(code)}' : Code: ${code}`);
  }
  ...
  readNumber(startsWithDot) {
    ...
    let octal = this.state.pos - start >= 2 && this.input.charCodeAt(start) === digit0;
  ...
  readString(quote) {
    ...
    for (;;) {
      ...
      if (ch === backslash) {
        ...
        chunkStart = this.state.pos;
      } else if (ch === lineSeparator || ch === paragraphSeparator) {
        ++this.state.pos;
        ++this.state.curLine;
      } else if (isNewLine(ch)) {
        this.raise(this.state.start, "Unterminated string constant");
    ...
    switch (ch) {
      ...
      default:
        if (ch >= digit0 && ch <= digit7) {
          ...
          if (octalStr !== "0" || next === digit8 || next === digit9) {
            if (inTemplate) {
...
class StatementParser extends ExpressionParser {
  ...
  isLet(context) {
    ...
    if (nextCh === leftSquareBracket) return true;
    if (context) return false;
    if (nextCh === leftCurlyBrace) return true;
  ...
  parseBreakContinueStatement(node, keyword) {
    const isBreak = keyword === breakKeyword;
    this.next();
